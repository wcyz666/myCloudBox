...
Cloud_client(char ** argv)
{
	...
	int error, flag, i, mynum, servernum, tmp, y,firstconn = 1, number, iResult;
	FILE *fp;
...
Cloud_locallist(Mylist mylist[])
{
	...
	if ((handle = (_findfirst(path, &FILEinfo))) != -1)	
	{
		...
		while (_findnext(handle, &FILEinfo) == 0)
		{
			...
			mylist[j].writetime = FILEinfo.time_write;
...
{ 
	int result = 0;
	OutputDebugString("Service Started.");
	return(result); 
}
...
{ 
   ...
   ServiceTable[0].lpServiceName = "CloudProbe";
